        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="card card-success">
                    ...
                    <div class="card-body">
                        <form action="{{ route('dashboard.rooms.update', [$room->id, $property_id]) }}" method="POST" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <input name="property_id" value="{{ $property_id }}" type="hidden" />
                            <input name="title" value="{{ old('title') ?? $room->title }}" class="form-control" type="text" placeholder="title">
                            @error('title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                            <br>
                            <textarea class="form-control" name="description" cols="30" rows="10">{{ old('description') ?? $room->title }}</textarea>
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                            <br>
